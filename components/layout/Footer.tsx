...
export function Footer() {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      // Dispatch directly to Google Sheets via universal telemetry engine
      const success = await sheetSync.newsletter({
        emailAddress: cleanEmail,
        subscriptionSource: typeof window !== 'undefined' ? window.location.pathname : 'footer_wire',
        consentGiven: true,
        status: 'SUBSCRIBED',
      });
      ...
      setErrorMessage(err?.message || 'Failed to link dispatch. Please try again.');
    } finally {
  ...
  return (
    <footer className="w-full border-t border-white/10 bg-[#040507] relative z-10 font-sans text-white selection:bg-cyan-500/30">
      ...
      <div className="border-b border-white/5 bg-gradient-to-r from-cyan-950/20 via-black/40 to-purple-950/20 backdrop-blur-xl relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 py-10 sm:py-12 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            
            {/* Left: Heading & Value Prop */}
            <div className="lg:col-span-6 space-y-2.5 text-left">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/25 text-cyan-300 text-[11px] font-mono tracking-wider uppercase shadow-[0_0_12px_rgba(6,182,212,0.15)]">
                <Sparkles className="w-3.5 h-3.5 text-cyan-400 animate-pulse" />
                <span>SOVEREIGN INTELLIGENCE WIRE // LIVE SYNC</span>
              </div>
            ...
            {/* Right: Interactive Subscription Form Connected to Sheets */}
              ...
              <form onSubmit={handleSubscribe} className="space-y-2.5">
                <div className="flex flex-col sm:flex-row items-center gap-2.5">
                  <div className="relative w-full">
                    ...
                    <input
                      ...
                      className="w-full pl-11 pr-4 py-3.5 rounded-2xl bg-white/[0.04] border border-white/10 text-xs font-mono text-white placeholder:text-neutral-500 focus:outline-none focus:border-cyan-400/70 focus:bg-white/[0.06] transition shadow-inner disabled:opacity-50"
                    />
                  </div>
                  <button
                    type="submit"
                    disabled={isSubmitting || !emailInput.trim()}
                    className="w-full sm:w-auto px-7 py-3.5 rounded-2xl bg-gradient-to-r from-white via-neutral-100 to-neutral-200 hover:from-white hover:to-white disabled:opacity-40 disabled:cursor-not-allowed text-black font-display font-bold text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-2 cursor-pointer shadow-[0_0_25px_rgba(255,255,255,0.3)] shrink-0 active:scale-95 hover:scale-[1.02]"
                  >
                    {isSubmitting ? (
                      <>
                ...
                {subscribed ? (
                  <p className="text-[11px] font-mono text-emerald-400 flex items-center gap-1.5 pt-1">
                    ...
                    <span>Transmitted to Ledger & Connected to Sheets! Welcome to the Wire.</span>
                  </p>
                ) : errorMessage ? (
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-[10px] font-mono text-neutral-500 pt-1">
                    <span className="flex items-center gap-1 text-emerald-400/90">
                      <Lock className="w-3 h-3 text-emerald-400" /> Zero Spam
                    </span>
                    <span>•</span>
