      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          <div className="p-5 rounded-3xl bg-black/60 border border-white/15 space-y-4 shadow-xl">
            ...
            {/* WhatsApp 1-Click Invite Generator */}
            <button
              type="button"
              onClick={() => {
                const randomCode = `zen-${Math.floor(100 + Math.random() * 900)}-${Math.floor(100 + Math.random() * 900)}`;
                const inviteUrl = typeof window !== 'undefined' ? `${window.location.origin}/call/${randomCode}` : `https://zenvitra.xyz/call/${randomCode}`;
                const text = encodeURIComponent(`Hey! Join my live video/voice chamber on Zenvitra:\n${inviteUrl}\nMode: ${selectedMode}`);
                window.open(`https://api.whatsapp.com/send?text=${text}`, '_blank');
              }}
              className="w-full py-2.5 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-xs font-semibold flex items-center justify-center gap-2 transition cursor-pointer"
            >
              <WhatsAppIcon className="w-4 h-4 text-emerald-400" />
              <span>Generate Room &amp; Share via WhatsApp</span>
            </button>
            ...
            {/* Speed-Dial Chamber Shortcuts */}
            <div className="pt-2 border-t border-white/10 space-y-2">
              <div className="flex items-center gap-1.5 text-[10px] font-mono uppercase tracking-wider text-slate-400">
                <Compass className="w-3.5 h-3.5 text-cyan-400" />
                <span>Instant Speed-Dial Chambers</span>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                {[
                  { id: 'zen-unsc-chamber', name: 'UNSC Plenary', mode: 'COMMITTEE', icon: '🇺🇳' },
                  { id: 'zen-diplomacy-lounge', name: 'Bilateral Lounge', mode: 'CALL', icon: '🤝' },
                  { id: 'zen-press-briefing', name: 'Press Studio', mode: 'EVENT', icon: '🎙️' },
                  { id: 'zen-climate-working-group', name: 'Climate Taskforce', mode: 'GROUP', icon: '🌱' }
                ].map((room) => (
                  <button
                    key={room.id}
                    type="button"
                    onClick={() => router.push(`/call/${room.id}?mode=${room.mode}&mic=${isMicOn}&cam=${isCamOn}&bg=${selectedBg}`)}
                    className="p-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-left transition-all hover:scale-[1.02] flex items-center gap-2 cursor-pointer"
                  >
                    <span className="text-base">{room.icon}</span>
                    <span className="text-[10px] font-semibold text-slate-200 truncate">{room.name}</span>
                  </button>
                ))}
              </div>
            </div>
